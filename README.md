...
###### 根据已知问题进行了调整
核心功能概述
这个脚本为AE脚本面板增加了一个“预设”视图，与原有的“脚本”视图并存，用户可以随时切换。在“预设”视图中，用户可以浏览、搜索、分类、管理和应用AE的动画预设。

### v1.1
###### 更新了预设管理界面
###### 三种应用模式（偏移和最后帧模式仅对位置，缩放等基础属性有用，其他蒙版，效果等都是正常动画）：
###### 基础模式：直接应用预设。
###### 偏移模式：在图层当前属性值的基础上，叠加预设的动画变化；比如预设动画是1s时间位置从（0,0）到（100,0），偏移模式第三秒给一个位置是（200,200）的图层应用，动画就是3-4秒位置从（200,200）到（300,200）。
###### 最后帧模式：让动画的结束状态与图层当前状态对齐，并反向推算动画的开始状态；比如预设动画是1s时间位置从（0,0）到（100,0），发光效果强度是0到1，在第3s给一个（200,200）的图层应用，动画就是2-3秒从（100,200）到（200,200），并且发光效果强度是2-3秒从0-1.

### v1.11
###### 修复了部分旧bug，调整了ui，增加了新bug
